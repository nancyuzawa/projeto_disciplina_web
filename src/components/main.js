...
function Main() {
  ...
  const processXMLData = (xmlDoc) => {
    ...
    for (let i = 0; i < cdItems.length; i++) {
      const name =
        cdItems[i].getElementsByTagName("NAME")[0]?.childNodes[0]?.nodeValue ||
        "N/A";
      const peso =
        cdItems[i].getElementsByTagName("PESO")[0]?.childNodes[0]?.nodeValue ||
        "N/A";
      const altura =
        cdItems[i].getElementsByTagName("ALTURA")[0]?.childNodes[0]
          ?.nodeValue || "N/A";
      const gordura =
        cdItems[i].getElementsByTagName("GORDURA")[0]?.childNodes[0]
          ?.nodeValue || "N/A";
      const imc =
        cdItems[i].getElementsByTagName("IMC")[0]?.childNodes[0]?.nodeValue ||
        "N/A";
